...
module.exports.update = async(req, res) => {
    ...
    await Club.findById(clubId, async function(err, doc) {
        ...
        doc.save().then(updatedClub).populate("leader").populate("treasurer").execPopulate().then((result) => {
            updatedClub.populate("leader").populate("treasurer").execPopulate().then((result) => {
                    res.status(200).send(ConvertClub(result))
                })
                .catch((err) => {
                    console.log(err);
